...
const Register = () => { //whatever was needed in backend
  ...
  const resumeHandler = (e) => {
    const file = e.target.files[0]; //getting files
    setResume(file);
  };

  const { loading, isAuthenticated, error, message } = useSelector((state) => state.user); //slice name is user
  ...
  const handleRegsiter = (e) => {
    ...
    const formData = new FormData(); //GETTING DATA in FormData
    ...
    if (role === "Candidate") { //only for candidate
          ...
          <form onSubmit={handleRegsiter}>
            ...
            <div className="wrapper">
              ...
              <div className="inputTag">
                ...
                <div>
                  <input type="number" placeholder="1234567890" value={phone} onChange={(e) => setPhone(e.target.value)} />
                  <FaPhoneFlip />
                </div>
              </div>
            </div>

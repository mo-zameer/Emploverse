...
const TopNiches = () => {
  const services = [ //array of niches
    {
      id: 1,
  ...
  return (
    <section className="services">
      <h3>Top Niches</h3>
      <div className="grid">
        {services.map((element) => { //traversing the array
          return (
            <div className="card" key={element.id}>
              <h4>{element.service}</h4> {/*displays service */}
              <p>{element.description}</p> {/*displays description */}
            </div>
          );
        })}
      </div>

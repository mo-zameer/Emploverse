...
export const fetchRecruiterApplications = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:4000/api/v1/application/recruiter/getall`,
      {
...
export const fetchCandidateApplications = () => async (dispatch) => {
  dispatch(applicationSlice.actions.requestForMyApplications());
  try {
    const response = await axios.get(
      `http://localhost:4000/api/v1/application/candidate/getall`,
      {
        withCredentials: true,
      }
    );
